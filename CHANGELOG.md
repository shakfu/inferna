...
- `tests/test_regression_review.py`: 15 new regression tests pinning the recently-fixed behavior. Coverage includes `LlamaContext` / `WhisperContext` post-close `RuntimeError`, `LlamaVocab.tokenize` retry on undersized buffer (52k-char input), `LlamaSampler.add_grammar` with invalid grammar raises `ValueError`, `LlamaModelKvOverride.key/val_str` oversized input raises, and progress-callback abort semantics (returns `False` / raises).
